...
import { Subscription, PackageManager, Package, PackageOwner } from "../model/Subscription";
...
export class AdminSubscriptionController {
  @HttpMethod("POST", "System was unable to add a subscription package.")
  public static add(req: Request, res: Response): void {
    const pBody: ISubscription.Admin.Package.Body = req.body.subscription.package;
    const pack: Package = new Package(pBody.name, pBody.productsID, new Discount(pBody.discount));
    pack.setType("admin");
    pack.save();
  }
  ...
  public static remove(req: Request, res: Response): void {
    ...
    if (!pack)
      throw new ClientError("Client provided an invalid package.");
    else
      pack.delete();
  }
  ...
  public static update(req: Request, res: Response): void {
    ...
    if (pack)
      ...
      throw new ClientError("Client subscription id doesn't exist");
  }
...
export class ClientSubscriptionController {
  ...
  public static add(req: Request, res: Response) {
     ...
     */
    const pBody: ISubscription.Client.Package.Body = req.body.subscription.package;
  ...
  public static remove(req: Request, res: Response): void {
    ...
    if (customer) {
      ...
      if (pack) {
        ...
        throw new ClientError("Client provided an invalid package id.");
    }
    else
    ...
    if (customer) {
      ...
      if(pack)
        if (pack.getType() === "client")
          ...
          throw new ClientError("Client has no permission to delete an admin created subscription.")
      else
        throw new ClientError("Client provided an an invalid id.")
    }
    else
      throw new ClientError("Client provided an invalid package id.");
  }
