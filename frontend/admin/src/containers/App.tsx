...
export class App extends Component<AppProps, AppState> {
  ...
  login = (): void => {
    const nextState: AppState = {...this.state, login: true, profile: {username: "johndoe1", name: "John Doe", imageURL: this.state.profile.imageURL}};
    this.setState(nextState);
    App.contextType = React.createContext(nextState);
  }
  toggleSidePanel = (): void => {
    const nextState: AppState = {...this.state, sidePanel: !this.state.sidePanel};
    this.setState(nextState);
    App.contextType = React.createContext(nextState);
  }
  render() {
