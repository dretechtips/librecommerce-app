...
import {  PhotoUploadProps, PhotoUploadState } from '../interface/PhotoUpload.interface';
...
export class PhotoUpload extends Component<PhotoUploadProps, PhotoUploadState> {
  ...
  readPhotosFile = (files: File[]) => {
    ...
    this.setState({...this.state, size: photos.reduce((a, b) => a + b.size, 0)});
    photos.forEach((cur) => {
  ...
  filesToPhoto = (e: ProgressEvent<FileReader>) => {
    ...
    if(typeof e.target!.result ===  "string") {
      if(this.state.photos.find(cur => cur.src === e.target!.result) === undefined) {
        const photo: HTMLImageElement = new Image();
        photo.src = e.target.result;
        this.state.photos.unshift(photo);
      }
      this.setState({...this.state, size: this.state.size + ( e.target.result.length * 2 / 1000000)});
    }
  }
