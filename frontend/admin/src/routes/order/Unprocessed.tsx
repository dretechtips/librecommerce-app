...
import {
  UnprocessedState,
  UnprocessedUIProps
} from "../../interface/routes/Order.interface";
...
class Unprocessed extends Component<{}, UnprocessedState> {
  ...
  halt = (id: string) => {
    ...
    if (el) this.state.halted.elements.push(el);
    const aval = this.state.avaliable;
    this.setState({
      ...this.state,
      avaliable: {
        ...this.state.avaliable,
        elements: aval.elements.filter(x => x.id !== id)
      }
    });
  };
  complete = (id: string) => {
    ...
    this.setState({
      ...this.state,
      avaliable: { ...this.state.avaliable, elements: avalf },
      halted: { ...this.state.halted, elements: haltf }
    });
  };
  render() {
    if (this.state.details)
      return (
        <Redirect to={"/order/details/" + this.state.details} push></Redirect>
      );
    else
      return (
        <UnprocessedUI
...
function UnprocessedUI(props: UnprocessedUIProps) {
  ...
  const add = () => history.push("/order/archive/add");
  return (
    <React.Fragment>
